...
repo; `find` is a plain full-text search of the file; `propose` appends to
`specs/_masters/pending-entries.md`.
**Later:** the same three operations, served by MCP, with a system key.
...
via `propose`. **The process never writes to the master list itself.** The owner of the list applies
pending entries as a separate, reviewable change.

Two reasons, and the second is the binding one:

1. A run in progress must not mutate shared infrastructure that other briefs are reading.
2. **It has to survive the MCP move.** A mechanism that appends to a file breaks the moment the
   master list is behind a call and is not writable in place. `propose` is the same operation either
   way — queue locally today, post a proposal later — and the brief never waits on it.
